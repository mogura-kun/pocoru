  ...
  return new Promise(resolve=>{
    _geminiQueue=_geminiQueue.then(async()=>{
      ...
      for(let i=0;i<=retry;i++){
        try{
          const res=await fetch(url,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contents:[{parts:[{text:prompt}]}],generationConfig:{maxOutputTokens:maxTokens,thinkingConfig:{thinkingBudget:0}}})});
      ...
      resolve(null);
    }).then(()=>new Promise(r=>setTimeout(r,2200)));
  });
}
...
function DetailModal({item,isOwn,onClose,onHeart,myHearts,onUpdate,onDelete,onViewUser,onEdit,myAvatar}){
  ...
  async function handleFlip(){
    ...
    if(!deepMsg&&!loadingDeep){
      setLoadingDeep(true);
      try{
        const prompt=`「${item.ai_msg}」の話題を深掘りして。同じ${cl(item.category)}に関する続き・背景・面白いエピソードを3文・120文字以内で返して。口調は基本敬語で優しく癒される雰囲気。ときどきため口がぽろっと漏れる感じ。絶対に3文以内・120文字以内。前置き不要。`;
      ...
      <div onClick={e=>e.stopPropagation()} style={{width:"100%",maxWidth:430,margin:"0 auto",background:"#f4f6f3",borderRadius:"28px 28px 0 0",padding:"22px 20px 48px",boxShadow:"0 -6px 30px rgba(0,0,0,0.10)",animation:"slideUp 0.3s ease",maxHeight:"90dvh",overflowY:"auto"}}>
        ...
        {item.ai_msg&&(
          <div style={{marginBottom:24}}>
            {!flipped
              ?<div onClick={handleFlip} style={{background:stickyBg,padding:"16px 14px 12px",boxShadow:"2px 3px 6px rgba(0,0,0,0.09)",fontFamily:font,fontSize:13,lineHeight:1.7,color:"#3a3028",borderRadius:2,cursor:"pointer",position:"relative"}}>
                <div style={{position:"absolute",top:0,left:"50%",transform:"translateX(-50%)",width:22,height:5,background:"rgba(0,0,0,0.08)",borderRadius:"0 0 4px 4px"}}/>
                <p style={{margin:"0 0 8px 0"}}>{item.ai_msg}</p>
                <div style={{textAlign:"right",fontSize:11,color:"#aaa"}}>めくる ›</div>
              </div>
              :<div style={{background:"#bae6fd",borderRadius:4,padding:"16px 14px 12px",boxShadow:"2px 3px 8px rgba(0,0,0,0.09)",fontFamily:font,fontSize:13,lineHeight:1.7,color:"#3a3028"}}>
                {loadingDeep
                  ?<p style={{margin:"0 0 8px 0",color:"#888"}}>読み込み中…</p>
                  :<p style={{margin:"0 0 8px 0",whiteSpace:"pre-wrap"}}>{deepMsg||"　"}</p>
                }
                <button onClick={e=>{e.stopPropagation();setFlipped(false);}} style={{fontSize:11,color:"#aaa",border:"none",background:"none",cursor:"pointer",marginTop:4,padding:0,fontFamily:font}}>‹ 戻る</button>
              </div>
            }
          </div>
        )}
...
function isInAppBrowser(){
  ...
  if(/Line\/|LineBrowser|Instagram|FBAN|FBAV|FB_IAB|Twitter\/|MicroMessenger|kakaotalk|Snapchat|GSA\/|YJApp|NaverApp|Puffin/.test(ua))return true;
...
function InAppWarning(){
  const [copied,setCopied]=useState(false);
  function copyUrl(){navigator.clipboard?.writeText(window.location.href).then(()=>{setCopied(true);setTimeout(()=>setCopied(false),2500);}).catch(()=>{});}
  return(
    <div style={{width:"100%",maxWidth:300,background:"#fff8e1",border:"1.5px solid #f5c842",borderRadius:14,padding:"18px 16px",textAlign:"center"}}>
      <div style={{fontSize:22,marginBottom:8}}>⚠️</div>
      <div style={{fontSize:13,fontWeight:700,color:"#7a5f00",marginBottom:6}}>このブラウザではログインできません</div>
      <div style={{fontSize:12,color:"#a08020",lineHeight:1.7,marginBottom:14}}>LINEやInstagramなどのアプリ内ブラウザでは<br/>Googleログインがブロックされます。<br/><b>SafariまたはChromeで開いてください。</b></div>
      <button onClick={copyUrl} style={{width:"100%",padding:"11px 0",borderRadius:10,border:"none",background:copied?"#83b195":"#3a3028",color:"white",fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:font}}>
        {copied?"✓ コピーしました！":"URLをコピーする"}
      </button>
      <div style={{fontSize:11,color:"#aaa",marginTop:8}}>コピーしたURLをSafariのアドレスバーに貼り付けてください</div>
    </div>
  );
}
    ...
    <div style={{minHeight:"100dvh",background:"#f4f6f3",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:font,padding:32}}>
      ...
      {iosInApp||showHelp?(
        ...
        <>
          ...
          <button onClick={()=>setShowHelp(true)} style={{marginTop:16,border:"none",background:"none",fontSize:12,color:"#aaa",cursor:"pointer",fontFamily:font,textDecoration:"underline"}}>ログインできない場合はこちら</button>
        </>
      )}
    </div>
  );
...
export default function App(){
  ...
  async function handleSave({note,category,emoji,photo,photoEdit,lat,lng,customTime}){
    ...
    try{
      ...
      const prompt=`今は${_m}月です。「${cl(category)}」に関する豆知識や感想を1文・40文字以内で返して。${note&&note!=="📷"?`発見メモ:「${note}」を参考に。`:""}${_m}月らしさを自然に取り入れて。口調は基本敬語で優しく癒される雰囲気。ときどきため口がぽろっと漏れる感じ。絶対に1文だけ・40文字以内。`;
