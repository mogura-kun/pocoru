...
function MoodColorPicker({color,onChange,onClose}){
  ...
  return(
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.45)",zIndex:500,display:"flex",alignItems:"flex-end"}}>
      <div onClick={e=>e.stopPropagation()} style={{width:"100%",maxWidth:430,margin:"0 auto",background:"#f5ecec",borderRadius:"24px 24px 0 0",padding:"24px 20px 44px",animation:"slideUp 0.3s ease"}}>
...
function SlideMenu({open,onClose,onSetTab,onOpenProfile,onSignOut,onCaptureLater,userName,avatarUrl}){
  return(
    <>
      ...
      <div style={{position:"fixed",top:0,right:0,bottom:0,width:260,background:"#f5ecec",zIndex:201,transform:open?"translateX(0)":"translateX(100%)",transition:"transform 0.3s cubic-bezier(0.4,0,0.2,1)",boxShadow:"-3px 0 20px rgba(0,0,0,0.09)",display:"flex",flexDirection:"column"}}>
...
function DetailModal({item,isOwn,onClose,onHeart,myHearts,onUpdate,onDelete,onViewUser,onEdit}){
  ...
  return(
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(58,48,40,0.5)",zIndex:350,display:"flex",alignItems:"flex-end"}}>
      <div onClick={e=>e.stopPropagation()} style={{width:"100%",maxWidth:430,margin:"0 auto",background:"#f5ecec",borderRadius:"28px 28px 0 0",padding:"22px 20px 48px",boxShadow:"0 -6px 30px rgba(0,0,0,0.10)",animation:"slideUp 0.3s ease",maxHeight:"90dvh",overflowY:"auto"}}>
...
function PostForm({initialData={}, laterMode=false, userLocation, locStatus, onSave, onClose, onDelete, saveLabel="みんなに届ける 🌱", title="発見を記録する ✨"}){
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(58,48,40,0.6)",zIndex:300,display:"flex",alignItems:"flex-end"}}>
      <div style={{width:"100%",maxWidth:430,margin:"0 auto",padding:"18px 18px 36px",background:"#f5ecec",borderRadius:"28px 28px 0 0",animation:"slideUp 0.3s ease",maxHeight:"92dvh",overflowY:"auto"}}>
...
function WeatherPanel({userLocation,onPost,onClose}){
  ...
  return(
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.3)",zIndex:250,display:"flex",alignItems:"flex-end"}}>
      <div onClick={e=>e.stopPropagation()} style={{width:"100%",maxWidth:430,margin:"0 auto",background:"#f5ecec",borderRadius:"24px 24px 0 0",padding:"20px 20px 40px",animation:"slideUp 0.3s ease"}}>
...
function ProfileModal({myUserId,myUserName,myAvatar,targetUserId,targetUserName,discoveries,token,onClose,onViewUser,onItemClick}){
  ...
  if(showFollowList){
    return(
      <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(58,48,40,0.5)",zIndex:300,display:"flex",alignItems:"flex-end"}}>
        <div onClick={e=>e.stopPropagation()} style={{width:"100%",maxWidth:430,margin:"0 auto",background:"#f5ecec",borderRadius:"28px 28px 0 0",padding:"22px 20px 48px",boxShadow:"0 -6px 30px rgba(0,0,0,0.10)",animation:"slideUp 0.35s ease",maxHeight:"90dvh",overflowY:"auto"}}>
  ...
  return(
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(58,48,40,0.5)",zIndex:300,display:"flex",alignItems:"flex-end"}}>
      <div onClick={e=>e.stopPropagation()} style={{width:"100%",maxWidth:430,margin:"0 auto",background:"#f5ecec",borderRadius:"28px 28px 0 0",padding:"22px 20px 0",boxShadow:"0 -6px 30px rgba(0,0,0,0.10)",animation:"slideUp 0.35s ease",maxHeight:"90dvh",overflowY:"auto"}}>
        <div style={{width:40,height:4,background:"#e0d8d0",borderRadius:2,margin:"0 auto 16px"}}/>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}>
...
function LoginScreen(){
  return(
    <div style={{minHeight:"100dvh",background:"#f5ecec",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:font,padding:32}}>
...
export default function App(){
  ...
  if(!authReady)return <div style={{minHeight:"100dvh",background:"#f5ecec",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:12,fontFamily:font}}><div style={{fontSize:44}}>🌱</div><div style={{fontSize:12,color:"#aaa"}}>読み込み中…</div></div>;
  ...
  return(
    <div style={{height:"100dvh",background:"#f5ecec",fontFamily:font,color:"#3a3028",display:"flex",flexDirection:"column",maxWidth:430,margin:"0 auto",overflow:"hidden"}}>
      ...
      {tab===0&&(
        <div style={{position:"fixed",top:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:430,height:"100dvh",display:"flex",flexDirection:"column",zIndex:10,background:"#f5ecec"}}>
          <div style={{flexShrink:0,paddingTop:"env(safe-area-inset-top,44px)",background:"rgba(245,236,236,0.98)",borderBottom:"1px solid rgba(0,0,0,0.08)"}}>
          ...
          <div style={{flexShrink:0,background:"rgba(245,236,236,0.98)",borderTop:"1px solid rgba(0,0,0,0.08)",display:"flex",alignItems:"center",padding:`10px 24px env(safe-area-inset-bottom,16px)`}}>
      ...
      {tab!==0&&(
        <>
          <div style={{position:"sticky",top:0,zIndex:30,background:"#f5ecec",borderBottom:"1px solid #e8e0d8"}}>
            ...
            <div style={{display:"flex",gap:5,padding:"0 12px 10px",overflowX:"auto"}}>
              {CATEGORIES.map(c=>{const on=visibleCats.includes(c.value);return <button key={c.value} onClick={()=>toggleCat(c.value)} style={{flexShrink:0,width:30,height:30,borderRadius:9,border:"none",cursor:"pointer",background:on?"#f5ecec":"rgba(0,0,0,0.05)",opacity:on?1:0.5,boxShadow:on?"0 1px 4px rgba(0,0,0,0.12)":"none",display:"flex",alignItems:"center",justifyContent:"center"}}><MotifIcon motif={c.value} color={on?c.defaultColor:"#aaa"} size={16}/></button>;})}
            </div>
          </div>
      ...
      {showAI&&(
        <div onClick={()=>setShowAI(false)} style={{position:"fixed",inset:0,background:"rgba(58,48,40,0.5)",zIndex:300,display:"flex",alignItems:"flex-end"}}>
          <div onClick={e=>e.stopPropagation()} style={{width:"100%",maxWidth:430,margin:"0 auto",padding:"26px 22px 46px",background:"#f5ecec",borderRadius:"28px 28px 0 0",animation:"slideUp 0.4s ease"}}>
