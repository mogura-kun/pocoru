...
function isInAppBrowser(){
  ...
  if(/Line\/|Instagram|FBAN|FBAV|Twitter|MicroMessenger|kakaotalk|Snapchat/.test(ua))return true;
...
function LoginScreen(){
  ...
  const [copied,setCopied]=useState(false);
  function copyUrl(){navigator.clipboard?.writeText(window.location.href).then(()=>{setCopied(true);setTimeout(()=>setCopied(false),2000);});}
  return(
    <div style={{minHeight:"100dvh",background:"#f4f6f3",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:font,padding:32}}>
      ...
      {inApp?(
        <div style={{width:"100%",maxWidth:300,background:"#fff8e1",border:"1.5px solid #f5c842",borderRadius:14,padding:"18px 16px",textAlign:"center"}}>
          ...
          <div style={{fontSize:12,color:"#a08020",lineHeight:1.7,marginBottom:14}}>LINEやInstagramなどのアプリ内ブラウザでは<br/>Googleログインがブロックされます。<br/>SafariまたはChromeで開いてください。</div>
          ...
          <div style={{fontSize:11,color:"#aaa",marginTop:8}}>コピーしたURLをSafariに貼り付けてください</div>
        </div>
      ):(
