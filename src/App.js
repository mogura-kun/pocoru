  ...
  async function handleFlip(){
    ...
    if(!deepMsg&&!loadingDeep){
      ...
      try{
        ...
        const response=await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemma2-9b-it:generateContent?key=${process.env.REACT_APP_GEMINI_KEY}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contents:[{parts:[{text:prompt}]}]})});
  ...
  async function handleSave({note,category,emoji,photo,photoEdit,lat,lng,customTime}){
    ...
    try{
      ...
      const response=await fetch(
        `https://generativelanguage.googleapis.com/v1beta/models/gemma2-9b-it:generateContent?key=${process.env.REACT_APP_GEMINI_KEY}`,
        {
          method:"POST",
