...
function MotifIcon({motif,color,size=24,shadow=false}){
  ...
  return <svg width={size} height={size} viewBox="0 0 24 24" fill={color} style={{filter:shadow?`drop-shadow(0 2px 4px ${color}88)`:"none",flexShrink:0,display:"block"}} dangerouslySetInnerHTML={{__html:s}}/>;
}
...
function LiveMap({discoveries,weatherReports,userLocation,visibleCats,onPinClick,centerMeRef}){
  ...
  useEffect(()=>{
    ...
    discoveries.filter(d=>d.lat&&d.lng&&visibleCats.includes(d.category)).forEach(d=>{
      ...
      const icon=L.divIcon({className:"",html:`<div style="opacity:${op};width:44px;height:44px;display:flex;align-items:center;justify-content:center"><svg width="38" height="38" viewBox="0 0 24 24" fill="${color}" style="filter:drop-shadow(0 2px 6px rgba(0,0,0,0.45))">${svgInner}</svg></div>`,iconSize:[44,44],iconAnchor:[22,22]});
